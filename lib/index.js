 ...
 * @param {boolean} useFakeTime  
 */
function setup(iosp, Kp, Ki, Kd, POn, controllerDirection, useFakeTime = false) {
  my_iosp = iosp;
  inAuto = false;
  ...
  usingFakeTime = useFakeTime;
  lastTime = millis(usingFakeTime) - SampleTime;
}
...
function compute() {
  ...
  const now = millis(usingFakeTime);
  const timeChange = (now - lastTime);
  if (timeChange >= SampleTime) {
    /*Compute all the working error variables*/
    const input = my_iosp.input;
    const error = my_iosp.setpoint - input;
...
function setOutputLimits(min, max) {
  ...
  if (inAuto) {
    if (my_iosp.output > outMax) my_iosp.output = outMax;
    else if (my_iosp.output < outMin) my_iosp.output = outMin;
 ...
 */
function initialize() {
  outputSum = my_iosp.output;
  lastInput = my_iosp.input;
  if (outputSum > outMax) outputSum = outMax;
  else if (outputSum < outMin) outputSum = outMin;
}
/**
...
function getMode() { return inAuto ? Mode.AUTOMATIC : Mode.MANUAL; }
